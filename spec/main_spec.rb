...
describe GameLogic do
  ...
  describe '#test_move' do
    it "returns true if given an intergure between 1 & 9 that isn't included inside taken turns array" do
      @taken_turns = [1, 2, 3]
      expect(test_move(4)).to be true
    end

    it 'returns false if given an intergure between 1 & 9 that is included inside taken turns array' do
      @taken_turns = [1, 2, 3]
      expect(test_move(3)).to be false
    end

    it 'returns false if given an intergure larger than 9' do
      expect(test_move(10)).to be false
    end

    it 'returns false if given an intergure lower than 1' do
